export default {
  ...
  mounted() {
    ...
    // Init Ad slot
    googletag.cmd.push(() => {
      this.debug && console.log('Ad Mounted, defineSlot: ', divId);
      const adSlot = googletag
        .defineSlot(adUnitPath, this.formattedSize, divId)
        .addService(googletag.pubads());

      if (setTargeting.length === 2) {
        const [key, val] = setTargeting;
        adSlot.setTargeting(key, val);
      } else if(setTargeting.length > 2) {
        // split into arrays of 2
        const _setTargeting = setTargeting.reduce((all,one,i) => {
            const chunk = Math.floor(i/2); 
            all[chunk] = [...(all[chunk]||[]),one]
            return all
        }, [])
        _setTargeting.forEach(([key, val]) => {
          adSlot.setTargeting(key, val);
        })
      }
  ...
  beforeDestroy() {
    ...
    this.debug && console.log('Ad beforeDestroy, destroySlot: ', this.divId);
    // Destroy ad slot
    googletag.cmd.push(() => {
